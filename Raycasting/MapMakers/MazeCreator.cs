...
namespace xnafan.MazeCreator
{
    ...
    public class MazeCreator
    {
        ...
        public Bitmap ConvertToBitmap(int tileSizeInPixels, bool showStartAndEnd = false, bool showGridlines = false)
        {
            int extraPixelIfGridIsShown = showGridlines ? 1 : 0;

                //create a new bitmap with a pixel extra for each tile +1 for drawing a grid around all tiles
                int width = (tileSizeInPixels + extraPixelIfGridIsShown) * this.Width + extraPixelIfGridIsShown;
                int height = (tileSizeInPixels + extraPixelIfGridIsShown) * this.Height + extraPixelIfGridIsShown;
            ...
            for (int y = 0; y < this.Height; y++)
            {
                ...
                {
                    char mapTile = mazeString[x + y * this.Width];
                    if (showStartAndEnd)
                    {
                        switch (mapTile)
                        {
                    ...
                    //draw the tile in
                    g.FillRectangle(fillBrush, extraPixelIfGridIsShown + x * (tileSizeInPixels + extraPixelIfGridIsShown), extraPixelIfGridIsShown + y * (tileSizeInPixels + extraPixelIfGridIsShown), tileSizeInPixels, tileSizeInPixels);
